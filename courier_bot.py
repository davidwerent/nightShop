...
async def contact(message: types.Message):
    ...
    try:
        cursor.execute('INSERT INTO user(user_id, phone) VALUES (?,?)', new_user)
        conn.commit()
...
async def show_details(message: types.Message):
    ...
    print(active_order)
    if active_order[0] == 0:
        await message.answer('У вас нет назначенного заказа! Список заказов /details')
    else:
        ...
        for item in request:
            ...
            mes += f"Цена за ед: {item.get('price')} руб\n\n"
            counter += 1
        keyboard = types.InlineKeyboardMarkup()
